...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    if (!loading && !promoLoading && categories && promotionalCategories) {
      ...
      Object.keys(categories).forEach((catId) => {
        const p = categories[catId].products.find(
      ...
      if (!foundProduct) {
        Object.keys(promotionalCategories).forEach((catId) => {
          const p = promotionalCategories[catId].products.find(
            (item) => item.id.toString() === productId,
          );
          if (p) {
            foundProduct = p;
  ...
  const handleInquiry = () => {
    navigate("/", {
      state: { prefillInquiry: `Interested in: ${product.title}` },
    });
    setTimeout(() => {
    ...
    <div className="min-h-screen bg-white pt-24 pb-16">
      <div className="container mx-auto px-6">
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-24">
            <h3 className="font-serif text-3xl text-primary mb-10 text-center">
              You May Also Like
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {relatedProducts.map((p) => (
                ...
                >
                  ...
                  <h4 className="font-serif text-lg text-primary text-center group-hover:text-accent transition-colors">
                    {p.title}
                  </h4>
