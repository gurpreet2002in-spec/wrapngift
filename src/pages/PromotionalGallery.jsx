...
const PromotionalGallery = () => {
  ...
  const allProducts = Object.values(promotionalCategories || {}).flatMap(
    (cat) =>
    ...
    <div className="pt-32 pb-24 bg-pearl min-h-screen">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20 max-w-2xl mx-auto">
          <span className="text-highlight font-bold uppercase tracking-[0.3em] text-[10px] mb-4 block underline decoration-royalty/20 underline-offset-8">
            Curated Collection
          </span>
          <h1 className="text-5xl md:text-6xl font-serif text-royalty mb-6">
            Promotional Gifts
          </h1>
          <p className="text-royalty/60 font-serif italic text-lg leading-relaxed">
            A definitive collection of quality promotional items and corporate
            gifts.
          </p>
        </div>
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
          {allProducts.map((item, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-royalty/80 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8 backdrop-blur-[2px]">
                <span className="text-highlight text-[10px] font-bold uppercase tracking-[0.2em] mb-2">
                  {item.categoryTitle}
                </span>
                <h3 className="text-white font-serif text-xl border-t border-white/20 pt-4">
                  {item.title}
                </h3>
              </div>
          ...
          {allProducts.length === 0 && (
            <div className="text-center w-full col-span-full py-20 text-gray-500 italic">
              No promotional gifts available yet.
            </div>
          )}
        </div>
      </div>
