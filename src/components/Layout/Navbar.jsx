    ...
    >
      <div className="container mx-auto px-6 flex justify-end lg:justify-between items-center relative h-16">
        ...
        <div className="hidden lg:flex flex-grow justify-center space-x-8 items-center pl-40">
          {(content.nav_links || []).map((navItem) => {
            if (navItem.type === "link") {
              return (
                <Link
                  key={navItem.id}
                  to={navItem.path}
                  className="text-[12px] font-bold tracking-[0.15em] uppercase text-secondary hover:text-primary transition-colors"
                >
                  {navItem.label}
                </Link>
              );
            ...
            if (navItem.type === "products_dropdown") {
              return (
                <div
                  key={navItem.id}
                ...
                >
                  <button className="flex items-center gap-1 text-[12px] font-bold tracking-[0.15em] uppercase text-secondary hover:text-primary transition-colors outline-none">
                    ...
                    <ChevronDown
                      size={14}
                      className={`transition-transform duration-300 ${productMenuOpen ? "rotate-180" : ""}`}
                    />
                  </button>

                  <div
                    className={`absolute top-full -left-4 w-64 bg-white shadow-2xl border border-gray-50 rounded-xl py-4 transition-all duration-300 origin-top ${productMenuOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}
                  >
            ...
            if (navItem.type === "promotional_dropdown") {
              return (
                <div
                  key={navItem.id}
                  className="relative group"
                ...
                >
                  <button className="flex items-center gap-1 text-[12px] font-bold tracking-[0.15em] uppercase text-secondary hover:text-primary transition-colors outline-none">
                    ...
                    <ChevronDown
                      size={14}
                      className={`transition-transform duration-300 ${promoMenuOpen ? "rotate-180" : ""}`}
                    />
                  </button>

                  <div
                    className={`absolute top-full -left-4 w-64 bg-white shadow-2xl border border-gray-50 rounded-xl py-4 transition-all duration-300 origin-top ${promoMenuOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}
                  >
                    <div className="absolute top-0 left-8 -translate-y-2 w-4 h-4 bg-white rotate-45 border-l border-t border-gray-50" />
        ...
        <div className="flex items-center space-x-8 text-secondary">
          <div className="hidden md:flex items-center gap-2 group cursor-pointer">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] uppercase tracking-widest text-gray-400 leading-none mb-1">
                Call Us
              </span>
              <span className="text-[11px] font-bold">
                +91 {content.contact_whatsapp}
              </span>
            </div>
          </div>
      ...
      {isOpen && (
        <div className="lg:hidden bg-white absolute top-full left-0 w-full shadow-lg py-6 flex flex-col items-center space-y-4 border-t border-gray-100 animate-in slide-in-from-top-4 duration-300">
          {(content.nav_links || []).map((navItem) => {
            ...
            if (navItem.type === "products_dropdown") {
              return (
                <div key={navItem.id} className="w-full px-8 pb-2">
                  <span className="block text-gray-400 text-[10px] uppercase tracking-[0.2em] mb-3 text-center">
                    {navItem.label}
                  </span>
